...
# this is a fix for Windows users. Without it, javascript files will be
# served with text/html content-type and the browser will not show any UI
mimetypes.init()
...
def update_token_counter(text, steps, styles, *, is_positive=True):
    ...
    token_count, max_length = max(
        [get_prompt_lengths_on_ui(prompt) for prompt in prompts], key=lambda args: args[0]
    )
...
def create_ui():
    ...
    with gr.Blocks(analytics_enabled=False, head=canvas_head) as txt2img_interface:
        ...
        with gr.Tab(
            "Generation", id="txt2img_generation"
                ...
                for category in ordered_ui_categories():
                    if category == "prompt":
                        ...
                        with gr.Row(elem_id="txt2img_accordions", elem_classes="accordions"):
                            with InputAccordion(
                                False, label="Hires. fix", elem_id="txt2img_hr", visible=True
                            ) as enable_hr:
                                ...
                                with FormRow(
                                    elem_id="txt2img_hires_fix_row_cfg", variant="compact"
                                ):
                                ...
                                with FormRow(
                                    ...
                                    variant="compact",
                                    visible=shared.opts.hires_fix_show_sampler,
                                ) as hr_checkpoint_container:
                                    ...
                                    def refresh_model_and_modules():
                                        ...
                                        return gr.update(
                                            choices=["Use same checkpoint"]
                                            + modules.sd_models.checkpoint_tiles(use_short=True)
                                        ), gr.update(choices=modules_list)

                                    hr_additional_modules = gr.Dropdown(
                                        label="Hires VAE / Text Encoder",
                                        elem_id="hr_vae_te",
                                    ...
                                    hr_checkpoint_refresh.click(
                                        ...
                                        outputs=[hr_checkpoint_name, hr_additional_modules],
                                        show_progress=False,
                                    )

                                with FormRow(
                                    elem_id="txt2img_hires_fix_row3b",
                                    variant="compact",
                                    visible=shared.opts.hires_fix_show_sampler,
                                ...
                                with FormRow(
                                    ...
                                    variant="compact",
                                    visible=shared.opts.hires_fix_show_prompts,
            ...
            txt2img_upscale_inputs = (
                txt2img_inputs[0:1]
                + [output_panel.gallery, dummy_component_number, output_panel.generation_info]
                + txt2img_inputs[1:]
            )
    ...
    with gr.Blocks(analytics_enabled=False, head=canvas_head) as img2img_interface:
        ...
        with gr.Tab(
            "Generation", id="img2img_generation"
        ) as img2img_generation_tab, ResizeHandleRow(equal_height=False):
            with ExitStack() as stack:
                ...
                def add_copy_image_controls(tab_name, elem):
                    with gr.Row(variant="compact", elem_id=f"img2img_copy_to_{tab_name}"):
                        for title, name in zip(
                            ["to img2img", "to sketch", "to inpaint", "to inpaint sketch"],
                    ...
                    if category == "image":
                        with gr.Tabs(elem_id="mode_img2img"):
                            ...
                            with gr.TabItem(
                                ...
                                init_img = ForgeCanvas(
                                    elem_id="img2img_image", height=512, no_scribbles=True
                                )
                                add_copy_image_controls("img2img", init_img)

                            with gr.TabItem(
                                "Sketch", id="img2img_sketch", elem_id="img2img_img2img_sketch_tab"
                            ) as tab_sketch:
                                sketch = ForgeCanvas(
                        ...
                        with FormRow():
                            with gr.Column(elem_id="img2img_column_size", scale=4):
                                ...
                                with gr.Tabs(elem_id="img2img_tabs_resize"):
                                    with gr.Tab(
                                        label="Resize to", id="to", elem_id="img2img_tab_resize_to"
                                    ) as tab_scale_to:
                                        with FormRow():
                                            with gr.Column(elem_id="img2img_column_size", scale=4):
                                                width = gr.Slider(
                                    ...
                                    with gr.Tab(
                                        label="Resize by", id="by", elem_id="img2img_tab_resize_by"
                                    ) as tab_scale_by:
                                        scale_by = gr.Slider(
                                            minimum=0.05,
                                            maximum=4.0,
                        ...
                        with gr.Row():
                            distilled_cfg_scale = gr.Slider(
                                ...
                                label="Distilled CFG Scale",
                                value=3.5,
                                elem_id="img2img_distilled_cfg_scale",
                            )
                            cfg_scale = gr.Slider(
                                minimum=1.0,
                                maximum=30.0,
            ...
            parameters_copypaste.add_paste_fields(
                "inpaint", init_img_with_mask.background, img2img_paste_fields, override_settings
            )
            parameters_copypaste.register_paste_params_button(
                parameters_copypaste.ParamBinding(
                    paste_button=toprow.paste,
